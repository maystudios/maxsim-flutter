...
function generateErrorRecoverySkill(): string {
  return [
    ...
    'user-invocable: true',
    'description: >-',
    '  Guide through error recovery escalation. Use when stuck on an error, build failure,',
    '  or test failure. Triggers on: error, stuck, failing, blocked, can\'t fix, help debug.',
    '---',
    '',
    '# Error Recovery Escalation',
    '',
    '## Tier 1: Self-Correction (try first)',
    '1. Read the **exact** error message — don\'t skim',
    '2. Check the last 3 changes you made — one likely caused it',
    '3. Run diagnostic commands:',
    '   - `flutter analyze` — static analysis',
    '   - `flutter test --name "failing_test"` — isolate the failure',
    '   - `dart fix --apply` — auto-fix common issues',
    '4. If build-related: `flutter clean && flutter pub get`',
    '5. If codegen-related: `dart run build_runner build --delete-conflicting-outputs`',
    '',
    '## Tier 2: AI-to-AI Escalation (after 2 failed attempts)',
    '- Ask another agent for a fresh perspective',
    '- Provide: exact error, file path, what you already tried',
    '- A fresh context window often spots what you missed',
    '- Use a subagent to investigate without polluting your context',
    '',
    '## Tier 3: Human-Augmented (after 3 total attempts)',
    '- Ask the user via AskUserQuestion:',
    '  - "I\'m stuck on [error]. I\'ve tried [X, Y, Z]. Could you provide additional context about [specific area]?"',
    '  - Request: domain knowledge, API documentation, business rules',
    '',
    '## Tier 4: Full Human Takeover',
    '- When the issue requires:',
    '  - Physical device testing or hardware access',
    '  - Third-party service configuration (Firebase console, API keys)',
    '  - Production environment access',
    '- Hand off with: error description, reproduction steps, all files involved, what was tried',
  ].join('\n');
}
...
function generateSddWorkflowSkill(): string {
  return [
    ...
    'user-invocable: true',
    'description: >-',
    '  Run the Specify-Design-Plan-Implement workflow for a feature. Triggers on:',
    '  SDD workflow, specify feature, plan implementation, new feature workflow.',
    '---',
    '',
    '# SDD Workflow: Specify → Plan → Implement',
    '',
    '## Phase 1: Specify',
    '- Gather requirements from the user or PRD story',
    '- Define domain entities using freezed sealed classes',
    '- Write acceptance criteria with testable predicates',
    '- Identify which Clean Architecture layers are affected',
    '- Output: a completed spec (see `/spec-template` skill)',
    '',
    '## Phase 2: Plan',
    '- Break the spec into implementation phases',
    '- Assign file ownership per phase',
    '- Estimate complexity (S/M/L) per file',
    '- Identify dependencies between phases',
    '- Output: a completed plan (see `/plan-template` skill)',
    '',
    '## Phase 3: Implement (TDD)',
    '- For each phase in the plan:',
    '  1. **RED**: Write failing tests for the phase\'s acceptance criteria',
    '  2. **GREEN**: Write minimal code to pass the tests',
    '  3. **REFACTOR**: Clean up while keeping tests green',
    '  4. Run `flutter analyze && flutter test`',
    '  5. Commit: `feat: [Story-ID] - phase description`',
    '',
    '## Phase 4: Validate',
    '- Run the full quality gate: `flutter analyze && flutter test --coverage`',
    '- Verify all acceptance criteria from the spec are met',
    '- Run the reviewer agent for compliance check',
    '- Mark the story as `passes: true` in prd.json',
  ].join('\n');
}

function generateSpecTemplateSkill(context: ProjectContext): string {
  const apiSection = context.modules.api
    ? [
        '',
        '### API Endpoints',
  ...
  const authSection = context.modules.auth
    ? [
        '',
        '### Auth Requirements',
        '- Authentication required: yes/no',
        '- Required roles/permissions',
        '- Auth state handling (logged in, logged out, expired)',
      ].join('\n')
    : '';

  const i18nSection = context.modules.i18n
    ? [
        '',
        '### Internationalization',
        '- User-facing strings to localize',
        '- Locale-sensitive formatting (dates, numbers)',
      ].join('\n')
    : '';

  const offlineSection = context.modules.database
    ? [
        '',
        '### Offline Support',
        '- Data to cache locally',
        '- Sync strategy (optimistic, queue-based)',
        '- Conflict resolution',
      ].join('\n')
    : '';
  ...
  return [
    ...
    '',
    '## Domain Entities',
    '- List all freezed entities this feature introduces or modifies',
    '- Include field names, types, and validation rules',
    '',
    '## Repository Interfaces',
    '- List abstract repository methods needed',
...
function generatePlanTemplateSkill(): string {
  return [
    ...
    '',
    '## Feature',
    '<!-- Reference the spec this plan implements -->',
    '',
    ...
    '| File | Action | Complexity | Owner |',
    '|------|--------|------------|-------|',
    '| `lib/features/<name>/domain/entities/<entity>.dart` | Create | S | builder |',
    '| `lib/features/<name>/domain/repositories/<repo>.dart` | Create | S | builder |',
    '',
    '## Phase 2: Data Layer',
    '| File | Action | Complexity | Owner |',
    '|------|--------|------------|-------|',
    '| `lib/features/<name>/data/models/<model>.dart` | Create | M | builder |',
    '| `lib/features/<name>/data/datasources/<source>.dart` | Create | M | builder |',
    '| `lib/features/<name>/data/repositories/<repo>_impl.dart` | Create | M | builder |',
    '',
    '## Phase 3: Presentation Layer',
    '| File | Action | Complexity | Owner |',
    '|------|--------|------------|-------|',
    '| `lib/features/<name>/presentation/providers/<name>_provider.dart` | Create | M | builder |',
    '| `lib/features/<name>/presentation/pages/<name>_page.dart` | Create | L | builder |',
    '',
    '## Phase 4: Integration',
    ...
    '- Phase 2 depends on Phase 1 (implements repository interfaces)',
    '- Phase 3 depends on Phase 1 (uses domain entities)',
...
function generateDebugWorkflowSkill(): string {
  return [
    ...
    'user-invocable: true',
    'description: >-',
    '  Systematic debugging workflow for Flutter apps. Use when debugging test failures,',
    '  runtime errors, or unexpected behavior. Triggers on: debug, why is this failing,',
    '  investigate, root cause, fix this error.',
    '---',
    '',
    '# Debug Workflow',
    '',
    '## Step 1: Reproduce',
    '- Run the exact failing command: `flutter test --name "test_name"`',
    '- Note the exact error message, file, and line number',
    '- Check if it\'s deterministic (run 3 times)',
    '- If intermittent: suspect async timing, shared state, or race conditions',
    '',
    '## Step 2: Isolate',
    '- Narrow the scope:',
    '  - Single test: `flutter test --name "exact_test_name"`',
    '  - Single file: `flutter test test/path/to_test.dart`',
    '  - Single assertion: add print statements around the failure',
    '- Check for shared state: does the test pass when run alone but fail in suite?',
    '- Check imports: is the right file/class being used?',
    '',
    '## Step 3: Identify Root Cause',
    '- Read the error message LITERALLY — it usually tells you exactly what\'s wrong',
    '- Common Flutter causes:',
    '  - `LateInitializationError` — provider not initialized in test setup',
    '  - `No MediaQuery widget ancestor` — missing `MaterialApp` wrapper in widget test',
    '  - `type \'Null\' is not a subtype of type \'X\'` — missing mock setup',
    '  - `MissingPluginException` — platform channel not mocked',
    '- Use `flutter test --reporter expanded` for verbose output',
    ...
    '- If the fix is in test setup, check all related tests for the same issue',
    '',
    '## Step 5: Verify',
    '- Run the previously failing test: should pass',
    '- Run the entire test file: no regressions',
    '- Run `flutter test`: full suite passes',
    '- Run `flutter analyze`: no new warnings',
    '',
    '## Flutter DevTools',
    '- `flutter run --debug` then open DevTools URL',
    '- Widget Inspector: check widget tree for unexpected nesting',
    '- Performance: check for unnecessary rebuilds',
